"""
Gevis Image Viewer — Windows Setup
Double-click Gevis_ImageViewer_Setup.exe to install.
Uninstall via Windows Settings → Apps → Gevis Image Viewer → Uninstall.
"""
...
if getattr(sys, "frozen", False):
    ...
    SRC = Path(__file__).parent / "dist" / "Gevis_ImageViewer"

DEST    = Path(os.environ["LOCALAPPDATA"]) / "Gevis Image Viewer"
EXE     = DEST / "Gevis_ImageViewer.exe"
SETUP   = DEST / "Gevis_ImageViewer_Setup.exe"
START   = Path(os.environ["APPDATA"]) / "Microsoft/Windows/Start Menu/Programs/Gevis Image Viewer"
...
def do_install(log):
    try:
        ...
        _shortcut(EXE, START / "Gevis Image Viewer.lnk")

        log("Creating Desktop shortcut…")
        _shortcut(EXE, DESKTOP / "Gevis Image Viewer.lnk")

        log("Registering file associations…")
        _reg(r"Software\Classes\Gevis.ImageFile", "", "Gevis Image Viewer")
        _reg(r"Software\Classes\Gevis.ImageFile\DefaultIcon", "", f"{EXE},0")
        _reg(r"Software\Classes\Gevis.ImageFile\shell\open\command", "", f'"{EXE}" "%1"')
        for ext in EXTS:
            _reg(f"Software\\Classes\\{ext}", "", "Gevis.ImageFile")
            log(f"  {ext} → Gevis Image Viewer")

        if getattr(sys, "frozen", False):
            shutil.copy2(sys.executable, SETUP)

        log("Registering with Windows (Add/Remove Programs)…")
        key = r"Software\Microsoft\Windows\CurrentVersion\Uninstall\Gevis_ImageViewer"
        _reg(key, "DisplayName",     "Gevis Image Viewer")
        _reg(key, "DisplayVersion",  "1.0.0")
        _reg(key, "Publisher",       "Gevis")
        _reg(key, "DisplayIcon",     str(EXE))
        _reg(key, "InstallLocation", str(DEST))
        _reg(key, "UninstallString", UNREG)
        _reg(key, "NoModify",        "1")
        _reg(key, "NoRepair",        "1")
        ...
        log("   Uninstall: Windows Settings → Apps → Gevis Image Viewer")
        return True
...
def do_uninstall(log):
    try:
        ...
        for p in [DESKTOP / "Gevis Image Viewer.lnk",
                  START   / "Gevis Image Viewer.lnk"]:
            if p.exists():
                p.unlink()
        try:
            START.rmdir()
        ...
        for sub in [
            r"Software\Classes\Gevis.ImageFile\shell\open\command",
            r"Software\Classes\Gevis.ImageFile\shell\open",
            r"Software\Classes\Gevis.ImageFile\shell",
            r"Software\Classes\Gevis.ImageFile\DefaultIcon",
            r"Software\Classes\Gevis.ImageFile",
            r"Software\Microsoft\Windows\CurrentVersion\Uninstall\Gevis_ImageViewer",
        ]:
            _del_reg(sub)
...
def _logo_path():
    if getattr(sys, "frozen", False):
        p = Path(sys._MEIPASS) / "resources" / "icons" / "logo_gevis.png"
    else:
        p = Path(__file__).parent / "resources" / "icons" / "logo_gevis.png"
    if not p.exists():
        # fallback to ico
        p = p.with_suffix(".ico")
...
class UninstallUI(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Gevis Image Viewer — Uninstall")
    ...
    def _build(self):
        ...
        tk.Label(self, text="Gevis Image Viewer",
    ...
    def _run(self):
        ...
        if ok:
            self.after(0, lambda: (
                messagebox.showinfo("Done", "Gevis Image Viewer has been removed."),
                self.destroy()
            ))
...
class SplashScreen(tk.Toplevel):
    def __init__(self, parent):
        ...
        tk.Label(self, text="Industrial Surface Inspection",
...
class InstallerUI(tk.Tk):
    ...
    def _show_main(self, splash):
        splash.destroy()
        self.title("Gevis Image Viewer — Setup")
        self.resizable(False, False)
        self.configure(bg="#0A0A0F")
    ...
    def _build(self):
        ...
        tk.Label(self, text="Version 1.0.0  —  Industrial Surface Inspection",
                 font=("Segoe UI", 10), fg="#888899", bg="#0A0A0F").pack()
    ...
    def _ask_launch(self):
        ans = messagebox.askyesno(
            "Setup Complete",
            "Gevis Image Viewer installed!\n\n"
            "• Desktop shortcut created\n"
            "• Appears in Start Menu — search 'Gevis'\n"
